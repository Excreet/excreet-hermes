<?php
/**
 * Plugin Name: Excreet Hermes Patch 3.2.2
 * Description: Re-runs product image attachment using direct Amazon CDN URLs
 *              (fixes failed sideload from local home_url). Resets 321 flag.
 * Version: 3.2.2
 */

if ( ! defined( 'ABSPATH' ) ) { exit; }

define( 'EX322_DONE_OPTION', 'excreet_322_images_cdn_done' );

add_action( 'init', 'excreet_322_attach_images_cdn', 30 );

function excreet_322_attach_images_cdn(): void {
    if ( get_option( EX322_DONE_OPTION ) ) { return; }
    ...
    /* Direct Amazon CDN image URL — reliable for sideloading */
    $map = [
    ...
    foreach ( $map as $slug => $asin ) {
        ...
        /* Remove existing thumbnail so we can replace it */
    ...
    update_option( EX322_DONE_OPTION, '1' );
}
