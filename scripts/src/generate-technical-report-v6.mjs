...
const outputPath = path.resolve(__dirname, '../../artifacts/hermes-ui/public/excreet-technical-report-v6.pdf');
...
const doc = new PDFDocument({
  ...
  bufferPages: true,
  info: {
    Title:   'Excreet — Technical Report v6',
    Author:  'Excreet',
...
doc.fillColor('#cccccc').font('Helvetica-Oblique').fontSize(14)
  .text('Technical Report  v6', { align: 'center', width: PW });
...
doc.fillColor(GOLD).font('Helvetica-Oblique').fontSize(9)
  .text('Pre-Clinical Cellular Health Intelligence \u2014 v6 Platform State', { width: PW })
  .moveDown(0.25);
...
body('v6 covers Phases 1\u201321 in full, adding three new phases beyond v5: (1) Member Guide \u2014 a gated inline reference page covering morning ritual, Vitality Score tiers, affiliate earnings, and cancellation steps; (2) Membership Pricing Page \u2014 auto-created /membership-options/ with large direct-to-checkout PMPro buttons fixing a broken onboarding flow; (3) PWA Installation \u2014 manifest.json, service worker, and apple-touch-icon delivered via patch-355, enabling home-screen installation on iOS and Android. Also recorded: Excreet Store naming unification across the Member Dashboard card and WordPress shop page, deploy-script touch-based OPcache bust, and the AI image model migration from DALL\u2011E 3 to gpt-image-1.');
rule();
...
body('Prior to v6 the WooCommerce shop page carried the title "Partner Picks \u2014 Trusted by Excreet" while the Member Dashboard card label read "Excreet Store." This inconsistency was corrected in two operations:');
bullet('Member Dashboard card (patch-297): label changed from "Partner Picks" to "Excreet Store"; CSS class ex297-card-title--store applied with color: #FFD700 (bright gold) and font-weight: 700 \u2014 distinguishing the store link visually from all other cards.');
bullet('WordPress shop page (page ID 882): title updated via WP-CLI (wp post update 882 --post_title="Excreet Store"). Slug retained as /shop/. Nginx cache purged for /shop/ immediately after.');
body('Both surfaces now use "Excreet Store" uniformly. The shop URL remains excreet.com/shop/.');
rule();

// ── OPcache Fix ──────────────────────────────────────────────────────────────
h2('Deploy Pipeline \u2014 OPcache Fixes (cumulative)');
body('Two OPcache issues have been identified and resolved across v5 and v6:');
...
body('opcache_reset() and opcache_invalidate() called from an HTTP-triggered flush script were returning false on SiteGround \u2014 likely due to opcache.restrict_api or web/CLI context isolation. The deploy script now runs a touch command via SSH immediately after SCP upload, updating the file\'s mtime. When opcache.validate_timestamps=1 (SiteGround default), PHP re-reads the file on the next request automatically.');
code('sshRun(host, user, port, key, `touch \'${REMOTE_DEST_DIR}/${REMOTE_DEST_NAME}\'`);', GREEN);
bullet('touch is non-fatal (wrapped in try/catch) so a permission failure does not abort the deploy.');
bullet('The HTTP-based opcache_reset() flush script remains as a secondary attempt before the nginx PURGE step.');
rule();
...
kv('Report gen:',   'node scripts/src/generate-technical-report-v6.mjs');
rule();

// ── Next Phase Candidates ────────────────────────────────────────────────────
h2('Next Phase Candidates (Phase 22+)');
const next = [
  ...
  ['TikTok video production',         'Symphony workflow: bathroom background (gpt-image-1) + URS-14 bottle on counter + Normal/Alarm phone overlays + voiceover avatar. 12-second script. Version A (avatar), B (voiceover only), C (animated Before/After \u2014 built in prep-video artifact).'],
...
doc.fillColor(LIGHTGRAY).font('Helvetica').fontSize(8).moveDown(0.15)
  .text('A PRE-CLINICAL WARNING SYSTEM.  \u00b7  Technical Report v6  \u00b7  Confidential',
    { align: 'center', width: PW });
...
for (let i = range.start; i < range.start + range.count; i++) {
  ...
  doc.fillColor(LIGHTGRAY).font('Helvetica').fontSize(7)
    .text(`Excreet \u2014 Technical Report v6  \u00b7  Confidential  |  Page ${pageNum} of ${total}`,
